...
namespace agregateurMetier
{
    public class TwitchLiveDAO
    {
        ...
        public List<TwitchLive> GetLivesFromEntryList(List<TwitchEntry> a_TwitchEntries) {
            List<TwitchLive> t_Lives = new List<TwitchLive>();

            foreach (TwitchEntry t_entry in a_TwitchEntries) {
                try {
                    TwitchLive t_live = new TwitchLive();

                    t_live.id = t_entry.Id;
                    t_live.ChannelLink = t_entry.Link;
                    ...
                    t_live.ChannelImageLink = t_entry.Image;
                    if (t_live.ChannelImageLink != string.Empty)
